    ...
    def can_make_request(self, estimated_tokens: int = 500) -> tuple[bool, Optional[str]]:
        ...
        with self.lock:
            ...
            if self.daily_requests >= self.rpd_limit_safe:
                return False, f"Daily request limit reached ({self.daily_requests}/{self.rpd_limit_safe})"
            
            # Check daily token limit
            if self.daily_tokens + estimated_tokens > self.tpd_limit_safe:
                return False, f"Daily token limit would be exceeded ({self.daily_tokens + estimated_tokens}/{self.tpd_limit_safe})"
            
            # Check requests per minute
            if len(self.request_times) >= self.rpm_limit_safe:
                return False, f"Rate limit: {len(self.request_times)}/{self.rpm_limit_safe} requests in last minute"
            
            # Check tokens per minute
            minute_tokens = sum(tokens for _, tokens in self.token_times)
            if minute_tokens + estimated_tokens > self.tpm_limit_safe:
                return False, f"Token limit: {minute_tokens + estimated_tokens}/{self.tpm_limit_safe} tokens in last minute"
    ...
    def wait_if_needed(self, estimated_tokens: int = 500) -> float:
        ...
        with self.lock:
            ...
            if len(self.request_times) >= self.rpm_limit_safe:
                ...
                wait_seconds = 60 - (time.time() - oldest_request) + 1  # +1 for safety margin
                if wait_seconds > 0:
