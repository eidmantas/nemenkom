...
def test_worker_handles_date_changes(temp_db):
    ...
    cursor.execute(
        ...
        VALUES (?, ?, ?, 'created'),
               (?, ?, ?, 'created')
    """,
        (calendar_stream_id1, "2026-01-08", "event1", calendar_stream_id1, "2026-01-22", "event2"),
    )
    conn.commit()
    ...
    cursor.execute(
        "SELECT calendar_synced_at FROM calendar_streams WHERE id = ?", (calendar_stream_id2,)
    )
    row = cursor.fetchone()
...
def test_worker_processes_multiple_groups(temp_db):
    ...
    with patch("services.calendar.get_google_calendar_service", return_value=mock_service):
        ...
        assert processed == len(calendar_stream_ids), (
            f"Should process {len(calendar_stream_ids)} streams"
        )
    ...
    """.format(",".join(["?"] * len(calendar_stream_ids))),
        calendar_stream_ids,
    )
