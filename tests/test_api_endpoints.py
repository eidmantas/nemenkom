...
def test_db_with_data():
    ...
    run_migrations(
        Path(db_path), Path(__file__).parent.parent / "services" / "scraper" / "migrations"
    )
    conn.commit()
    ...
    location_id = write_location_schedule(
        conn, "Avižienių", "Aleksandravas", "", test_dates, test_kaimai_str, None, "bendros"
    )
    conn.commit()
...
def test_db_with_village_and_streets():
    ...
    write_location_schedule(
        conn, "Test", "SimpleVillage", "", test_dates, "SimpleVillage", None, "bendros"
    )
...
def test_api_schedule_village_with_streets_requires_street(test_db_with_village_and_streets):
    ...
    try:
        ...
        with app.test_client() as client:
            ...
            response = client.get(
                "/api/v1/schedule?seniunija=Test&village=VillageWithStreets&street=Main Street"
            )
            assert response.status_code == 200
            data = response.get_json()
...
def test_api_schedule_street_with_house_numbers_requires_house_numbers(
    test_db_with_village_and_streets,
):
    ...
    try:
        ...
        with app.test_client() as client:
            ...
            response = client.get(
                "/api/v1/schedule?seniunija=Test&village=VillageWithStreets&street=Second Street"
            )
            assert response.status_code == 400
            data = response.get_json()
            assert "error" in data
            ...
            response = client.get(
                "/api/v1/schedule?seniunija=Test&village=VillageWithStreets&street=Second Street&house_numbers=1, 2, 3"
            )
            assert response.status_code == 200
            data = response.get_json()
            assert data["village"] == "VillageWithStreets"
...
def test_get_unique_villages_format(test_db_with_village_and_streets):
    ...
    try:
        ...
        test_villages = [
            v for v in villages if v["village"] in ["SimpleVillage", "VillageWithStreets"]
        ]
