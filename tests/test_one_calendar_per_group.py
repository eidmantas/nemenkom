...
def test_one_calendar_per_schedule_group(temp_db):
    ...
    with (
        ...
        assert mock_service.calendars().insert().execute.call_count == 1, (
            "Calendar should be created only once"
        )
...
def test_different_waste_types_different_calendars(temp_db):
    ...
    assert schedule_group_id1 != schedule_group_id2, (
        "Different waste types should have different schedule_group_ids"
    )
    assert calendar_stream_id1 != calendar_stream_id2, (
        "Different waste types should have different calendar streams"
    )
    ...
    with patch("services.calendar.get_google_calendar_service", return_value=mock_service):
        ...
        assert result1["calendar_id"] != result2["calendar_id"], (
            "Different waste types should have different calendars"
        )
...
def test_same_location_different_streets_same_calendar_if_same_schedule(temp_db):
    ...
    for location_id in location_ids:
        ...
        if schedule and schedule.get("calendar_id"):
            assert schedule["calendar_id"] == calendar_id, (
                "All locations should share same calendar"
            )
...
def test_calendar_not_recreated_on_date_change(temp_db):
    ...
    with (
        ...
        assert mock_service.calendars().insert().execute.call_count == 1, (
            "Calendar should not be recreated"
        )
...
def test_calendar_creation_only_for_schedule_groups_not_villages(temp_db):
    ...
    assert mock_service.calendars().insert().execute.call_count == 1, (
        "Should create only ONE calendar for all villages with same schedule"
    )
...
def test_no_duplicate_calendar_creation_on_retry(temp_db):
    ...
    with (
        ...
        for i in range(5):
            ...
            assert result["calendar_id"] == calendar_id1, (
                f"Retry {i + 1} should return same calendar"
            )
            assert result.get("existing") is True, (
                f"Retry {i + 1} should indicate existing calendar"
            )

        # Verify calendar was created only ONCE (not 6 times)
        assert mock_service.calendars().insert().execute.call_count == 1, (
            "Calendar should be created only once, even with multiple retries"
        )
