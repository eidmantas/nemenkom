...
def parse_xlsx(file_path: Path, year: int = 2026, skip_ai: bool = False) -> List[Dict]:
    ...
    logger.info(
        f"Parsing complete: {len(results)} entries in {total_time:.1f}s (AI: {ai_parse_count}, Traditional: {traditional_parse_count}, Skipped: {skipped_count})"
    )
    if skip_ai and skipped_count > 0:
        print(
            f"Parsed {len(results)} location entries (skipped {skipped_count} AI-needed entries) in {total_time:.1f}s"
        )
    else:
