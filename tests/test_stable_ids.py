...
def test_find_or_create_schedule_group_stable_id(temp_db):
    ...
    # Verify in database
    cursor = conn.cursor()
    cursor.execute("SELECT id, dates, dates_hash, calendar_synced_at FROM schedule_groups WHERE id = ?", (id1,))
    row = cursor.fetchone()
...
def test_find_or_create_schedule_group_no_change(temp_db):
    ...
    assert synced_at_after is not None, "calendar_synced_at should remain set when dates don't change"
