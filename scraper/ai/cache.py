...
class AIParserCache:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        if db_path is None:
            db_path = Path(__file__).parent.parent.parent / 'database' / 'waste_schedule.db'
    ...
    def set(self, kaimai_str: str, parsed_result: List, tokens_used: int = 0):
        ...
        serializable_result = [list(item) if isinstance(item, tuple) else item for item in parsed_result]
