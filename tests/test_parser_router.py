...
class TestShouldUseAIParser:
    ...
    def test_simple_village_no_ai(self):
        ...
        assert not should_use_ai_parser("Aleksandravas")
        assert not should_use_ai_parser("Aukštadvaris")
        assert not should_use_ai_parser("Tarandė")

    def test_village_with_streets_no_ai(self):
        """Village with streets in parentheses should NOT use AI"""
        assert not should_use_ai_parser("Avižieniai (Akacijų aklg., Avižų g.)")
        assert not should_use_ai_parser("Bendoriai (Ateities g., Bendorėlių arka)")

    def test_house_numbers_use_ai(self):
        """Entries with house numbers SHOULD use AI"""
        assert should_use_ai_parser("Gilužiai Trumpoji g. (Nr. 1-1, 1-2, 5)")
        assert should_use_ai_parser("Sudervės g. 26, 28")
        assert should_use_ai_parser("Ilgoji g.,nuo 18 iki 18U")

    def test_ordinal_street_names_no_ai(self):
        """Ordinal street names (1-oji g.) are NOT house numbers"""
        assert not should_use_ai_parser("Kalvių 1-oji, 2-oji, 3-oji g.")
        assert not should_use_ai_parser("Žemaitukų 1-oji g.")

    def test_mixed_ordinal_and_house_numbers_use_ai(self):
        """Mix of ordinal streets AND house numbers SHOULD use AI"""
        assert should_use_ai_parser("Pikutiškės (Kalvių 1-oji, 2-oji g., Sudervės g. 26, 28)")
    ...
    def test_streets_outside_parens_use_ai(self):
        ...
        assert should_use_ai_parser("Bendoriai (Ilgoji g.) Žalumos g.")
    ...
    def test_streets_without_parens_use_ai(self):
        ...
        assert should_use_ai_parser("Bezdonys Pakalnės g., Draugystės g.")
        # Should NOT match simple village
        assert not should_use_ai_parser("Bezdonys")
        # Should NOT match village with streets in parentheses
        assert not should_use_ai_parser("Bezdonys (Pakalnės g., Draugystės g.)")
